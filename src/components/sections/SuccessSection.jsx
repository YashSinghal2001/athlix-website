...
export default function SuccessSection({ id = "success", className = "" }) {
    ...
    // Note: The original Success.jsx had a useEffect to scroll into view on mobile.
    // For the homepage section version, we likely DON'T want auto-scroll on mount as it might hijack the landing experience.
    // I will comment it out or remove it for the Section version to ensure stability.
    /*
    useEffect(() => {
                    ...
                    <div className="relative z-10">
                        ...
                        {isMobile ? (
                            ...
                            <div className="mt-6 md:mt-12 relative">
                                        ...
                                        <div key={i} className="min-w-[85vw] snap-center">
                                            <Card className="p-0 overflow-hidden border border-brand-border h-full" hoverEffect={false}>
                                                ...
                                                <div className="p-5 bg-brand-surface">
                                                    ...
                                                    <p className="text-brand-text font-medium">{item.focus}</p>
                                                </div>
                                            </Card>
                                        </div>
                                    ))}
                                ...
                                <div className="flex justify-center gap-2 mt-2">
                                    {transformations.map((_, i) => (
                                        <motion.div key={i} className={`h-1.5 rounded-full transition-colors duration-300 ${i === activeIndex ? "bg-brand-accent" : "bg-gray-300"}`} animate={{ width: i === activeIndex ? 24 : 6 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} />
                                    ))}
                                </div>
                            </div>
                        ) : (
                            ...
                            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 mt-6 md:mt-12">
                                {transformations.map((item, i) => (
                                    <Card key={i} className="p-0 overflow-hidden border border-brand-border hover:border-brand-accent/30 transition-colors duration-300 group">
                                        <div className="h-64 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-gray-500 relative">
